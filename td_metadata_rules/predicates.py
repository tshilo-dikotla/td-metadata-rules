...
class Predicates(PredicateCollection):
    ...
    def func_mother_pos(self, visit):
    ...
    def func_mother_neg(self, visit):
        """Returns true if mother is hiv neg."""
        maternal_status_helper = MaternalStatusHelper()
        return maternal_status_helper.hiv_status == NEG

    def func_show_elisa_requisition_hiv_status_ind(self, visit):
...
#         return self.func_mother_pos(visit)
    ...
    def func_show_rapid_test_form(self, visit):
        subject_identifier = visit.subject_identifier
        maternal_status_helper = MaternalStatusHelper()
        if visit.timepoint == '2000M' and maternal_status_helper.hiv_status == NEG:
    ...
    def func_show_srh_services_utilization(self, visit):
        ...
        previous_maternal_contr = Reference.objects.filter(
            ...
            identifier=visit.subject_identifier).exclude(
                timepoint=visit.timepoint).order_by('-report_datetime').first()

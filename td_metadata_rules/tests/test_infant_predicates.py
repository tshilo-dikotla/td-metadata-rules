...
class TestInfantPredicates(SiteTestCaseMixin, TestCase):
    ...
    def setUp(self):
        ...
        self.maternal_reference_helper = self.reference_helper_cls(
            visit_model=self.maternal_visit_model,
            subject_identifier=self.maternal_subject_identifier)

        self.infant_reference_helper = self.reference_helper_cls(
            visit_model=self.visit_model,
            subject_identifier=self.subject_identifier)

        self.maternal_reference_helper.create_visit(
            report_datetime=report_datetime, timepoint='2000M')

        self.infant_reference_helper.create_visit(
...
#         maternal_status_helper = MaternalStatusHelper(status=POS)
#
#         self.infant_reference_helper.create_for_model(
...
#                                           maternal_status_helper))
    ...
    def test_func_show_infant_nvp_dispensing_required(self):
        ...
        self.maternal_reference_helper.create_for_model(
            report_datetime=self.maternal_visits[0].report_datetime,
            reference_name='td_maternal.maternalrando',
            visit_code='2000M',
            rx='NVP\n')

        self.assertTrue(
            pc.func_show_infant_nvp_dispensing(self.infant_visits[2],
                                               maternal_status_helper))

    def test_func_show_infant_nvp_dispensing_not_required(self):
        pc = InfantPredicates()
        maternal_status_helper = MaternalStatusHelper(status=POS)
    ...
    def test_func_show_infant_nvp_dispensing_not_required_2(self):
        pc = InfantPredicates()
        maternal_status_helper = MaternalStatusHelper(status=NEG)
    ...
    def test_func_show_nvp_adjustment_2010_required(self):
        ...
        self.infant_reference_helper.create_for_model(
            report_datetime=self.infant_visits[0].report_datetime,
    ...
    def test_func_show_nvp_adjustment_2010_not_required(self):
        pc = InfantPredicates()
        maternal_status_helper = MaternalStatusHelper(status=POS)
    ...
    def test_func_show_nvp_adjustment_2010_not_required_2(self):
        pc = InfantPredicates()
        maternal_status_helper = MaternalStatusHelper(status=NEG)
    ...
    def test_func_show_nvp_adjustment_2010_not_required_3(self):
        pc = InfantPredicates()
        maternal_status_helper = MaternalStatusHelper(status=POS)
        ...
        self.assertFalse(
            pc.func_show_nvp_adjustment_2010(self.infant_visits[2],
                                             maternal_status_helper))
    ...
    def maternal_visits(self):
        return LongitudinalRefset(
            subject_identifier=self.maternal_subject_identifier,
            visit_model=self.maternal_visit_model,
            name=self.maternal_visit_model,
            reference_model_cls=self.reference_model
        ).order_by('report_datetime')
